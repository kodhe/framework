...
/**
 * Unified Routing Manager dengan Hybrid Support dan Module Integration
 */
class RoutingManager
{
    ...
    public function __construct(?Router $router = null)
    {
        ...
        $this->setModernRouter(new ModernRouter());
    ...
    /**
     * Resolve routing dengan hybrid approach dan module support
     */
    public function resolve(Request $request): array
    {
        $uri = $this->getRequestUri($request);
        $cacheKey = md5($uri . '_hybrid_module');
        ...
        // Extract module info dari routing
        ...
        if (!$this->isValidRouting($routing) || !$this->controllerExists($routing)) {
            ...
            if ($override404) {
                $routing404 = $this->parse404Override($override404);
                $routing404 = $this->extractModuleInfo($routing404);
                $this->lastRouting = $routing404;
                return $routing404;
            }
            
            // Return default 404 routing dengan module info
            $errorRouting = $this->getErrorRouting();
            $errorRouting = $this->extractModuleInfo($errorRouting);
            $this->lastRouting = $errorRouting;
            return $errorRouting;
        }
        
        // Enrich routing details dengan module context
        $routing = $this->enrichRoutingWithModule($routing);
    ...
    protected function extractModuleInfo(array $routing): array
    {
        ...
        // Priority 1: Extract dari URI melalui Modules::parse_routes
        if (!empty($routing['uri'])) {
            $segments = explode('/', trim($routing['uri'], '/'));
            if (!empty($segments[0])) {
                $module = $this->extractModuleFromUri($segments[0]);
                if ($module) {
        ...
        // Priority 5: Coba dari segment pertama URI
        $uri = $routing['uri'] ?? '';
    ...
    /**
     * Extract module dari URI menggunakan Modules::parse_routes
     */
    protected function extractModuleFromUri(string $firstSegment): ?string
    {
        // Cek jika segment pertama adalah module yang valid
        if (Modules::moduleExists($firstSegment)) {
            return $firstSegment;
        }
        
        // Coba parse routes untuk melihat jika ada mapping
        $parsed = Modules::parse_routes($firstSegment, $firstSegment);
        if ($parsed && is_array($parsed) && !empty($parsed[0])) {
            // $parsed[0] seharusnya module name
            $module = $parsed[0];
    ...
    /**
     * Enrich routing dengan module context
     */
    protected function enrichRoutingWithModule(array $routing): array
    {
        // Jika sudah ada module, tambahkan info tambahan
        if (!empty($routing['module'])) {
            $module = $routing['module'];
            
            // Dapatkan module path dari Modules class
            $modulePath = Modules::path($module);
            if ($modulePath) {
                $routing['module_path'] = $modulePath;
                
                // Tambahkan controller path jika tipe legacy
                if ($routing['type'] === 'legacy' && empty($routing['file'])) {
            ...
            // Tambahkan module namespace untuk modern controllers
            if ($routing['type'] === 'modern' && empty($routing['namespace'])) {
                $routing['namespace'] = 'App\\Modules\\' . ucfirst($module) . '\\Controllers\\';
            }
            
            // Set module di Modules registry untuk akses global
    ...
    public function getModule(): string
    {
        ...
        // Coba dari modern router
        if ($this->modernRouter && property_exists($this->modernRouter, 'module')) {
            return $this->modernRouter->module ?? '';
        }
    ...
    {
        try {
            $modernRouter = $this->modernRouter ?? new ModernRouter();
            $routing = $modernRouter->matchRequest($request);

            if ($routing) {
                // Simpan modern router instance untuk eksekusi
                $routing['_router'] = $modernRouter;
    ...
    {
        // Jika ada module, coba cari via Modules class
        if (!empty($routing['module'])) {
    ...
    public function clearCache(): void
    {
        ...
        if ($this->modernRouter && method_exists($this->modernRouter, 'clearCache')) {
