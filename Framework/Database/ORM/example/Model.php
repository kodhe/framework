 ...
 */
class Model extends CIModel implements \ArrayAccess
{
    /**
    ...
    public function __get($name)
    {
        ...
        // ORM schema check
            ...
            // FIX: Perbaikan akses ke CI instance
            $ci = get_instance();
            
            // Cek jika ini adalah property dari CI instance (seperti db, load, dll)
            if (property_exists($ci, $name)) {
                return $ci->$name;
            }
            
            // Cek jika ini adalah class yang sudah diload di CI (melalui is_loaded())
            $loadedClasses = is_loaded();
            if (isset($loadedClasses[$name]) && isset($ci->$name)) {
                return $ci->$name;
            }
            
            // Cek khusus untuk 'db' karena sering digunakan
            if ($name === 'db' && isset($ci->db)) {
                return $ci->db;
            }

            // Cek melalui loader (mengikuti logika LegacyLoader)
            if (isset($ci->load) && method_exists($ci->load, '__get')) {
                // Coba akses melalui loader
                try {
                    return $ci->load->$name;
                } catch (\Exception $e) {
                    // Lewati exception dan lanjut ke error utama
                }
            }

            // Exception jika property benar-benar tidak ada
            throw new \Exception("Property `{$name}` does not exist", 500);  
        }
