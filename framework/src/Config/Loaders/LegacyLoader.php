...
use Kodhe\Framework\Database\Loader as DB;
...
class LegacyLoader
{
    ...
    protected function _get_modern_class_name($class)
    {
        ...
        $library_map = [
            'User_agent' => 'Kodhe\\Agent\\UserAgent',
            'Agent' => 'Kodhe\\Agent\\UserAgent',
            'Cache' => 'Kodhe\\Cache\\Cache',
            'Calendar' => 'Kodhe\\Calendar\\Calendar',
            'Cart' => 'Kodhe\\Cart\\Cart',
            'Database' => 'Kodhe\\Database\\Database',
            'DB' => 'Kodhe\\Database\\Database',
            'Driver' => 'Kodhe\\Driver\\Driver',
            ...
            'Form_validation' => 'Kodhe\\Validation\\FormValidation',
            'Validation' => 'Kodhe\\Validation\\FormValidation',
	...
	{
		if(!class_exists(DB::class)) return;

		return DB::database($params, $return, $query_builder);
	}
		...
		foreach ($helpers as &$helper)
		{
			...
			foreach ($this->_ci_helper_paths as $path)
			{
				...
				$try = [
					...
					$path . 'Framework' . DIRECTORY_SEPARATOR . 'Support' . DIRECTORY_SEPARATOR . 'Helpers' . DIRECTORY_SEPARATOR . $helper . '.php',
					$path . 'Support' . DIRECTORY_SEPARATOR . 'Helpers' . DIRECTORY_SEPARATOR . $helper . '.php',
				];

				$loaded = FALSE;
				foreach ($try as $filepath) {
					if (is_file($filepath)) {
						include_once($filepath);
						$this->_ci_helpers[$helper] = TRUE;
						log_message('info', 'Helper loaded: ' . $helper . ' (' . $filepath . ')');
						$loaded = TRUE;
						break;
					}
				}
			...
			// Last resort: resolve from framework package via Container reflection
			if ( ! isset($this->_ci_helpers[$helper]) && class_exists(\Kodhe\Framework\Container\Container::class, true)) {
				try {
					$ref = new \ReflectionClass(\Kodhe\Framework\Container\Container::class);
					$helpersDir = dirname($ref->getFileName(), 2) . DIRECTORY_SEPARATOR . 'Support' . DIRECTORY_SEPARATOR . 'Helpers' . DIRECTORY_SEPARATOR;
					$filepath = $helpersDir . $helper . '.php';
					if (is_file($filepath)) {
						include_once($filepath);
						$this->_ci_helpers[$helper] = TRUE;
						log_message('info', 'Helper loaded: ' . $helper . ' (' . $filepath . ')');
	...
	protected function _ci_load($_ci_data)
	{
		...
		if ( ! $file_exists && is_string($_ci_path) && $_ci_path !== "" && ! file_exists($_ci_path))
		{
