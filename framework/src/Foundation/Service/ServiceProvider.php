 ...
 * Core Provider
 */
class ServiceProvider extends ConcreteBinding
{
    /**
     ...
     */
    public function __construct(BindingInterface $delegate, $path, array $data)
    {
        $this->path = $path;
        $this->data = $data;
        ...
        if (!empty($aliases)) {
            foreach ($aliases as $origClassName => $aliasClassName) {
                if (is_numeric($origClassName)) {
                    ...
                    if (strpos($aliasClassName, 'Kodhe\Modules') === 0) {
                        $replace_once = 1;
                        $aliasClassName = str_replace('Kodhe\Modules', 'Modules', $aliasClassName, $replace_once);
                    }
                    $aliasClassName = 'Kodhe\\' . $aliasClassName;

                    if (!class_exists($aliasClassName) && class_exists($origClassName)) {
                        class_alias($aliasClassName, $origClassName);
                    }

                } else {
                    // $origClassName = alias (e.g. CI_Model), $aliasClassName = real class
                    if (!class_exists($origClassName) && class_exists($aliasClassName)) {
                        class_alias($aliasClassName, $origClassName);
                    }

                    // Linux is case-sensitive: also register common lowercase variant (CI_model)
                    if (strpos($origClassName, 'CI_') === 0) {
                        $lowerVariant = 'CI_' . strtolower(substr($origClassName, 3));
                        if ($lowerVariant !== $origClassName
                            && !class_exists($lowerVariant)
                            && class_exists($aliasClassName)
                        ) {
                            class_alias($aliasClassName, $lowerVariant);
                        }
                    }
                }
