...
namespace Kodhe\Library\Session;
...
class Session {
	...
	public function __construct(array $params = array())
	{
		...
		if ($class instanceof \Kodhe\Library\Session\HandlerInterface)
		{
			if (is_php('5.4'))
			{
	 ...
	 */
	 protected function _ci_load_classes($driver)
 	{
 		...
 		//interface_exists('Kodhe\Library\Session\HandlerInterface', FALSE) OR require_once(BASEPATH.'Libraries/Session/HandlerInterface.php');

 		$class = 'Kodhe\Library\Session\Drivers\\'.ucwords($driver);
		...
		if(class_exists($class)) {
 		...
 		return $this->_ci_load_classes_legacy($driver);
 	}

	protected function _ci_load_classes_legacy($driver)
	{
		...
		$prefix = config_item('subclass_prefix');
 		...
 		{
			if (file_exists($file_path = resolve_path(APPPATH,).'/Session/Session_driver.php'))
 			{
 				require_once($file_path);
 			}

 			if (file_exists($file_path = resolve_path(APPPATH,).'/Session/'.$prefix.'Session_driver.php'))
 			{
 				require_once($file_path);
 			}
 		}
		...
		if ( ! class_exists($class, FALSE) && file_exists($file_path = resolve_path(BASEPATH,).'/Session/Drivers/'.$class.'.php'))
		{
			require_once($file_path);
		...
		{
			if (file_exists($file_path = resolve_path(APPPATH,).'/Session/Drivers/'.$class.'.php') OR file_exists($file_path = BASEPATH.'libraries/Session/Drivers/'.$class.'.php'))
			{
				require_once($file_path);
			}
		...
		if ( ! class_exists($prefix.$class, FALSE) && file_exists($file_path = resolve_path(APPPATH,).'/Session/Drivers/'.$prefix.$class.'.php'))
		{
			require_once($file_path);
