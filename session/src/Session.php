...
class Session
{
	...
	public function __construct(array $params = array())
	{
		...
		if ($class instanceof \Kodhe\Framework\Session\HandlerInterface)
		{
			if (is_php('5.4'))
			{
	 ...
	 */
	 protected function _ci_load_classes($driver)
 	{
 		...
 		//interface_exists('Kodhe\Framework\Session\HandlerInterface', FALSE) OR require_once(BASEPATH.'Libraries/Session/HandlerInterface.php');

 		$class = 'Kodhe\Framework\Session\Drivers\\'.ucwords($driver);
	...
	protected function _ci_load_classes_legacy($driver)
	{
		...
		if ( ! class_exists($class, FALSE) && file_exists($file_path = resolve_path(BASEPATH,).'/Drivers/'.$class.'.php'))
		{
			require_once($file_path);
		...
		{
			if (file_exists($file_path = resolve_path(APPPATH,).'/Drivers/'.$class.'.php') OR file_exists($file_path = BASEPATH.'libraries/Drivers/'.$class.'.php'))
			{
				require_once($file_path);
			}
		...
		if ( ! class_exists($prefix.$class, FALSE) && file_exists($file_path = resolve_path(APPPATH,).'/Drivers/'.$prefix.$class.'.php'))
		{
			require_once($file_path);
